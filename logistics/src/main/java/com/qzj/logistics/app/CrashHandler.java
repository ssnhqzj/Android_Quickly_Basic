...
import com.qzj.logistics.utils.GeneralConfiguration;
import com.qzj.logistics.utils.Logger;
...
public class CrashHandler implements UncaughtExceptionHandler {  
    ...
    private boolean handleException(Throwable ex) {  
    	...
    	System.out.println("CrashHandler.handleException()");
        if (ex == null) {  
            return true;  
        }  
        // 使用Toast来显示异常信息  
    ...
    public void collectCrashDeviceInfo(Context ctx) {  
    	...
    	System.out.println("CrashHandler.collectCrashDeviceInfo()" );
        try {  
            // Class for retrieving various kinds of information related to the  
            // application packages that are currently installed on the device.  
            // You can find this class through getPackageManager().  
    ...
    private String saveCrashInfoToFile(Throwable ex) {  
    	...
    	Logger.i(TAG,"CrashHandler.saveCrashInfoToFile()");
        Writer info = new StringWriter();
        PrintWriter printWriter = new PrintWriter(info);  
        // printStackTrace(PrintWriter s)  
        // 将此 throwable 及其追踪输出到指定的 PrintWriter  
        ...
        try {  
            long timestamp = System.currentTimeMillis();  
            String fileName = GeneralConfiguration.APP_CACHE_PATH+"/Log/";
            // 保存文件  
//            FileOutputStream trace = mContext.openFileOutput(fileName, Context.MODE_PRIVATE);  
//            mDeviceCrashInfo.setProperty("id", "123");
//            mDeviceCrashInfo.store(trace, "uhuo");  
            if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {  
                ...
                if (dirs.length>5){
                    for (File f : dirs){
                        if (f.isFile() && new Date().getTime()/(1000 * 60 * 60 * 24)-(f.lastModified()/(1000 * 60 * 60 * 24))>=3){
                            f.delete();
                        }
                    }
                }
                FileOutputStream fos = new FileOutputStream(fileName + "crash-" + timestamp + CRASH_REPORTER_EXTENSION);  
...
//            trace.close();  
    ...
    private void sendCrashReportsToServer(Context ctx) {  
    	...
    	Logger.e(TAG,"CrashHandler.sendCrashReportsToServer()");
        String[] crFiles = getCrashReportFiles(ctx);
        if (crFiles != null && crFiles.length > 0) {  
            TreeSet<String> sortedFiles = new TreeSet<String>();  
            sortedFiles.addAll(Arrays.asList(crFiles));  
    ...
    private String[] getCrashReportFiles(Context ctx) {
        ...
        Logger.e(TAG, "CrashHandler.getCrashReportFiles()");
        File filesDir = ctx.getFilesDir();
        // 实现FilenameFilter接口的类实例可用于过滤器文件名  
        FilenameFilter filter = new FilenameFilter() {  
            // accept(File dir, String name)  
    ...
    private void postReport(File file) {
        ...
        Logger.e(TAG, "CrashHandler.postReport()");
        // TODO 使用HTTP Post 发送错误报告到服务器  
        // 这里不再详述,开发者可以根据OPhoneSDN上的其他网络操作  
        // 教程来提交错误报告  
    }  
    ...
    public void sendPreviousReportsToServer() {
        ...
        Logger.e(TAG, "CrashHandler.sendPreviousReportsToServer()");
        sendCrashReportsToServer(mContext);  
    }  
} 
