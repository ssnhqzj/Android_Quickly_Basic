...
public class PullRefreshLayout extends ViewGroup {
    ...
    // 此刷新控件的第二级可滑动子控件
    private View childScrollTarget;
    // 是否检查过第二级可滑动子控件
    private boolean isFindChildScrollTarget;
...
//        mRefreshView.bringToFront();
    ...
    private boolean canChildScrollUp() {
        if(!isFindChildScrollTarget){
            childScrollTarget = findChildScrollView();
            isFindChildScrollTarget = true;
        }
        ...
        if (android.os.Build.VERSION.SDK_INT < 14) {
            ...
            // 用于支持非直接子控件，仅支持第二级
            if (childScrollTarget != null){
                return ViewCompat.canScrollVertically(childScrollTarget, -1);
            } else {
    ...
    /**
     * 判断其子控件是否有ListView,ScrollView等可滑动的View
     */
    private View findChildScrollView() {
        int childCount = getChildCount();
        for (int i=0; i<childCount; i++) {
            View childView = getChildAt(i);
            if (childView instanceof ViewGroup) {
                for (int j=0; j<((ViewGroup) childView).getChildCount(); j++) {
                    View secondChildView = ((ViewGroup) childView).getChildAt(j);
                    if (secondChildView instanceof ListView
                            || secondChildView instanceof ScrollView
                            || secondChildView instanceof ExpandableListView
                            || secondChildView instanceof GridView) {
